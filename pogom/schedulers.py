...
class SpeedScan(HexSearch):
    ...
    def location_changed(self, scan_location, db_update_queue):
        ...
        initial = {}
        all_scans = {}
        for sl in ScannedLocation.select_in_hex(self.scan_location, config['STEP_LIMIT']):
        ...
        self.band_status()
        spawnpoints = SpawnPoint.select_in_hex(self.scan_location, config['STEP_LIMIT'])
        if not spawnpoints:
    ...
    def _generate_locations(self):
        ...
        # upper part
        for ring in range(1, config['STEP_LIMIT']):
            ...
            for i in range(ring + (ring + 1 < config['STEP_LIMIT'])):
                loc = get_new_coords(loc, xdist, WEST)
                results.append((loc[0], loc[1], 0))
